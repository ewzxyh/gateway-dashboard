    ...
    <div
        ...
        <div class="w-1/4">
            <div class="flex justify-center my-2 w-full">
                <h1 class="text-xl font-bold my-2">Editar Bandeira</h1>
            </div>
            <hr>
            <form method="post" action="{{ route('bandeiras.update', $bandeira->id) }}">
                @csrf
                @method('PUT')
                <div class="flex flex-col gep-y-2">
                    <label for="nome">Nome:</label>
                    <input value="{{ $bandeira->nome }}" class="px-2 h-10 border border-black" type="text"
