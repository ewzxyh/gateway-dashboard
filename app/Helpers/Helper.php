...
class Helper
{
    ...
    public static function adquirenteDefault($user_id = null)
    {
        \Log::info('Helper::adquirenteDefault - User ID recebido', ['user_id' => $user_id]);
        ...
        if ($user_id) {
            ...
            if ($user) {
                \Log::info('Helper::adquirenteDefault - Preferred adquirente', ['preferred' => $user->preferred_adquirente ?? 'NULL']);
                \Log::info('Helper::adquirenteDefault - Adquirente override', ['override' => $user->adquirente_override ? 'Sim' : 'Não']);
            }
            if ($user && $user->preferred_adquirente && $user->adquirente_override) {
                // Verificar se a adquirente preferida está ativa
                $adquirentePreferida = Adquirente::where('referencia', $user->preferred_adquirente)
                    ->where('status', 1)
                    ->first();
                if ($adquirentePreferida) {
                    \Log::info('Helper::adquirenteDefault - Retornando adquirente preferida', ['referencia' => $adquirentePreferida->referencia]);
                    return $adquirentePreferida->referencia;
                }
            }
        }
        
        // Fallback para adquirente padrão global do sistema
        $adquirente = Adquirente::where('is_default', 1)->first();
        \Log::info('Helper::adquirenteDefault - Adquirente padrão', ['referencia' => $adquirente ? $adquirente->referencia : 'NULL']);
