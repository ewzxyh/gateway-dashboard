...
{
    /**
     * Callback para depósitos (Cash-in) - BSPay
     */
    public function callbackDeposit(Request $request)
    {
        try {
            $data = $request->all();
            Log::debug('[+] Callback BSPay Deposit: ' . json_encode($data));
            ...
            // Formato direto BSPay
            if (isset($data['status']) && isset($data['idTransaction'])) {
                $status = $data['status'];
                $transactionId = $data['idTransaction'];
                $typeTransaction = $data['typeTransaction'] ?? 'PIX';
                Log::info("Callback BSPay: Formato direto detectado - Status: $status, TransactionId: $transactionId");
            }
            // Formato PIXUP (BSPay também pode usar)
            elseif (isset($data['requestBody']) && isset($data['requestBody']['external_id'])) {
                ...
                // Mapear status do formato PIXUP para BSPay
                ...
                Log::info("Callback BSPay: Formato PIXUP detectado - Status: $status, TransactionId: $transactionId");
            }
            
            if (!$status || !$transactionId) {
                Log::warning('Callback BSPay Deposit: Dados obrigatórios não encontrados');
            ...
            if (!$cashin) {
                ...
                if ($cashout) {
                    $isWithdraw = true;
                    Log::info("Callback BSPay: Transação identificada como SAQUE - ID: $transactionId");
                }
            } else {
                Log::info("Callback BSPay: Transação identificada como DEPÓSITO - ID: $transactionId");
            }

            if (!$cashin && !$cashout) {
                Log::warning("Callback BSPay: Solicitação não encontrada para idTransaction: $transactionId");
            ...
            if ($isWithdraw) {
                if ($cashout->status !== 'PENDING') {
                    Log::warning("Callback BSPay Withdraw: Solicitação já processada. Status atual: {$cashout->status}");
                    return response()->json(['status' => false, 'message' => 'Solicitação já processada']);
                }
            } else {
                if ($cashin->status !== 'WAITING_FOR_APPROVAL') {
                    Log::warning("Callback BSPay Deposit: Solicitação já processada. Status atual: {$cashin->status}");
                    return response()->json(['status' => false, 'message' => 'Solicitação já processada']);
                }
            }
            ...
            if (in_array(strtoupper($status), ['PAID', 'APPROVED'])) {
                if ($isWithdraw) {
                    Log::info("Callback BSPay Withdraw: Processando saque aprovado para transação: $transactionId");
                    ...
                    if ($cashout->status === 'PENDING') {
                        ...
                        Log::info("Callback BSPay Withdraw: Status atualizado para PAID_OUT para transação: $transactionId");
                        ...
                        if ($user) {
                            ...
                            if (!$taxaPorFora) {
                                ...
                                Log::info("Callback BSPay Withdraw: Descontando valor do saque (taxa não por fora):", [
                                ...
                                Log::info("Callback BSPay Withdraw: Taxa por fora - não descontando saldo novamente:", [
                                    'user_id' => $user->user_id,
                            ...
                            \App\Helpers\BalanceLogHelper::logSaqueOperation(
                                ...
                                [
                                    ...
                                    'operacao' => 'callbackBSPayWithdraw'
                                ]
                            );
                            
                            Log::info("Callback BSPay Withdraw: Saldo atualizado:", [
                                'user_id' => $user->user_id,
                        ...
                        Log::info("Callback BSPay Withdraw: Saque já processado anteriormente", [
                    ...
                    Log::info("Callback BSPay Withdraw: Saque processado com sucesso para transação: $transactionId");
                    
                    return response()->json(['status' => true, 'message' => 'Saque processado com sucesso']);
                } else {
                    Log::info("Callback BSPay Deposit: Processando depósito aprovado para transação: $transactionId");
                    ...
                    if ($user) {
                        ...
                        \App\Helpers\BalanceLogHelper::logDepositOperation(
                            ...
                            [
                                ...
                                'operacao' => 'bspay_callback_deposit'
                            ]
                        );

                        Log::info("Callback BSPay Deposit: Saldo incrementado para usuário {$user->user_id} - Valor: {$cashin->deposito_liquido}");
                        ...
                        if ($cashin->split_email && $cashin->split_percentage) {
                            ...
                            Log::info("Callback BSPay Deposit: Splits processados", [
                        ...
                        if (isset($user->gerente_id) && !is_null($user->gerente_id)) {
                            ...
                            if ($gerente) {
                                ...
                                if ($comissao > 0) {
                                    ...
                                    Log::info("Callback BSPay Deposit: Comissão paga para gerente {$gerente->user_id} - Valor: $comissao");
                                }
                            }
                        }
                    } else {
                        Log::error("Callback BSPay Deposit: Usuário não encontrado para user_id: {$cashin->user_id}");
                    }
                    ...
                    if ($order) {
                        ...
                        if (!is_null($user->webhook_url) && in_array('pago', (array) $user->webhook_endpoint)) {
                            ...
                            Log::info("Callback BSPay Deposit: Webhook enviado para: {$user->webhook_url}");
                        }
                    }
                    ...
                    if ($cashin->callback && $cashin->callback !== 'web') {
                        ...
                        Log::info("Callback BSPay Deposit: Callback personalizado enviado para: {$cashin->callback}");
                    }
                ...
                Log::info("Callback BSPay Deposit: Status não processado: $status para transação: $transactionId");
                return response()->json(['status' => false, 'message' => 'Status não processado']);
            }

        } catch (\Exception $e) {
            Log::error('Callback BSPay Deposit - Erro: ' . $e->getMessage());
            Log::error('Callback BSPay Deposit - Stack trace: ' . $e->getTraceAsString());
            return response()->json(['status' => false, 'message' => 'Erro interno do servidor'], 500);
        }
    }

    /**
     * Callback para saques (Cash-out) - BSPay
     */
    public function callbackWithdraw(Request $request)
    {
        try {
            $data = $request->all();
            Log::debug('[+] Callback BSPay Withdraw: ' . json_encode($data));

            // Verifica se é um evento de transferência PIX
            if (!isset($data['status']) || !isset($data['idTransaction'])) {
                Log::warning('Callback BSPay Withdraw: Dados obrigatórios não encontrados');
                return response()->json(['status' => false, 'message' => 'Dados obrigatórios não encontrados']);
            }
            ...
            if (!$cashout) {
                Log::warning("Callback BSPay Withdraw: Solicitação não encontrada para idTransaction: $transactionId");
                return response()->json(['status' => false, 'message' => 'Solicitação não encontrada']);
            }
            ...
            if (in_array($status, ['paid', 'approved', 'completed'])) {
                ...
                Log::info("Callback BSPay Withdraw: Saque rejeitado para transação: $transactionId - Status: $status");

                // PROTEÇÃO CRÍTICA: NUNCA reverter saldo se o saque já foi processado
                // Isso evita reversões indevidas quando BSPay envia callbacks duplicados
                    ...
                    if ($user) {
                        ...
                        \App\Helpers\BalanceLogHelper::logSaqueOperation(
                            ...
                            [
                                ...
                                'operacao' => 'bspay_callback_withdraw'
                            ]
                        );
                        
                        Log::info("Callback BSPay Withdraw: Saldo revertido para usuário {$user->user_id} - Valor: {$cashout->amount}");
                    }
                } else {
                    Log::warning("Callback BSPay Withdraw: Tentativa de rejeitar saque já processado IGNORADA", [
                        'transaction_id' => $transactionId,
                        'current_status' => $cashout->status,
                ...
                Log::info("Callback BSPay Withdraw: Status não processado: $status para transação: $transactionId");
                return response()->json(['status' => false, 'message' => 'Status não processado']);
            }

        } catch (\Exception $e) {
            Log::error('Callback BSPay Withdraw - Erro: ' . $e->getMessage());
            Log::error('Callback BSPay Withdraw - Stack trace: ' . $e->getTraceAsString());
            return response()->json(['status' => false, 'message' => 'Erro interno do servidor'], 500);
        }
    }

    /**
     * Teste do callback BSPay
     */
    public function testCallback(Request $request)
    {
        $data = $request->all();
        Log::info('[TEST] Callback BSPay Test: ' . json_encode($data));
        
        return response()->json([
            'status' => true,
            'message' => 'Callback BSPay testado com sucesso',
            'received_data' => $data,
    ...
    private function enviarCallbackSaque($cashout, $transactionId, $typeTransaction)
    {
        ...
        Log::info("Callback BSPay: Iniciando envio de callback para cassino", [
        ...
        while ($tentativa <= $maxTentativas) {
            try {
                Log::info("Callback BSPay: Tentativa {$tentativa}/{$maxTentativas}", [
                    'callback_url' => $callbackUrl,
                    'transaction_id' => $transactionId
                ]);
                ...
                Log::info("Callback BSPay: Resposta do cassino (tentativa {$tentativa})", [
                ...
                if ($response->successful()) {
                    Log::info("Callback BSPay: Callback enviado com sucesso para o cassino", [
                        'tentativa' => $tentativa,
                        'transaction_id' => $transactionId
                    ]);
                    return; // Sucesso, sair do loop
                } else {
                    Log::warning("Callback BSPay: Erro ao enviar callback para o cassino (tentativa {$tentativa})", [
                        'status_code' => $response->status(),
                        'response_body' => $response->body(),
                        'tentativa' => $tentativa,
                        'max_tentativas' => $maxTentativas
                ...
                Log::error("Callback BSPay: Exceção ao enviar callback para o cassino (tentativa {$tentativa})", [
                    'error' => $e->getMessage(),
                    'callback_url' => $callbackUrl,
                    'transaction_id' => $transactionId,
                    'tentativa' => $tentativa,
            ...
            if ($tentativa <= $maxTentativas) {
                ...
                Log::info("Callback BSPay: Aguardando {$delay}s antes da próxima tentativa");
                sleep($delay);
            }
        }

        // Se chegou aqui, todas as tentativas falharam
        Log::error("Callback BSPay: Todas as tentativas falharam após {$maxTentativas} tentativas", [
            'callback_url' => $callbackUrl,
            'transaction_id' => $transactionId,
            'max_tentativas' => $maxTentativas
        ]);
        ...
        // pois o BSPay já processou o saque com sucesso
        Log::info("Callback BSPay: Saque mantido como processado mesmo com falha em todas as tentativas de callback", [
            'transaction_id' => $transactionId,
            'reason' => 'BSPay já processou o saque com sucesso - falha no callback não afeta o pagamento'
        ]);
    }
}
